...
export default function IDCardsBulkGenerate({ users = [], school, filters }: Props) {
    ...
    return (
        <AppLayout>
            <Head title="Bulk ID Cards Print" />
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6 print-area">
                <div className="flex justify-between items-center no-print">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">Bulk Print Preview</h1>
                        <p className="text-gray-600">Review and print {users.length} ID cards at once</p>
                    </div>
                    <div className="flex space-x-2">
                        ...
                        <Button 
                            onClick={() => window.open(`/id-cards/bulk/print?type=${filters.type ?? ''}&class_name=${filters.class_name ?? ''}&search=${filters.search ?? ''}`, '_blank')} 
                            disabled={users.length === 0}
                        >
                            <Printer className="w-4 h-4 mr-2" />
                            Print All ({users.length})
                        </Button>
                    </div>
                </div>
                ...
                {users.length === 0 ? (
                    <div className="text-center py-12 text-gray-500 no-print">
                        <p>No cards available for printing with current filters.</p>
                    </div>
                ) : (
                    <div className="print-card-container grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-12 justify-items-center mt-6 print:block print:space-y-0 print:mt-0">
                        {users.map((user) => {
                            const theme = getTheme(user.type);
                            return (
                                <div key={user.id} className="print-card-wrapper">
                                    {/* CR-80 Card: 540px x 340px */}
                                    <div
                                        className="print-card-scale w-[540px] h-[340px] rounded-2xl overflow-hidden shadow-2xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-slate-100 flex flex-row relative select-none"
