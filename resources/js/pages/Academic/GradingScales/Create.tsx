...
export default function GradingScaleCreate() {
    const [gradeRows, setGradeRows] = useState<GradeRow[]>([
        { grade: 'A', min: 75, max: 100, points: 4, description: 'Excellent' },
        { grade: 'B', min: 65, max: 74, points: 3, description: 'Very Good' },
        { grade: 'C', min: 50, max: 64, points: 2, description: 'Good' },
        { grade: 'D', min: 40, max: 49, points: 1, description: 'Satisfactory' },
        { grade: 'F', min: 0, max: 39, points: 0, description: 'Fail' },
    ]);
    ...
    const loadPreset = (type: 'necta' | 'competency') => {
        if (type === 'necta') {
            setGradeRows([
                { grade: 'A', min: 75, max: 100, points: 4, description: 'Excellent' },
                { grade: 'B', min: 65, max: 74, points: 3, description: 'Very Good' },
            ...
            setData(prev => ({ ...prev, scale_type: 'necta' as string }));
        } else {
            setGradeRows([
                { grade: 'A', min: 90, max: 100, points: 4, description: 'Excellent' },
                { grade: 'B', min: 80, max: 89, points: 3, description: 'Good' },
                { grade: 'C', min: 70, max: 79, points: 2, description: 'Satisfactory' },
                { grade: 'D', min: 60, max: 69, points: 1, description: 'Needs Improvement' },
                { grade: 'F', min: 0, max: 59, points: 0, description: 'Below Standard' },
            ]);
            setData(prev => ({ ...prev, scale_type: 'competency' as string }));
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Set form grades and post
        const payload = { ...data, grades: gradesRecord };
        router.post('/academic/grading-scales', payload);
    };
        ...
        <AppLayout>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">Create Grading Scale</h1>
                        <p className="text-muted-foreground">Define standard thresholds and scale points.</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Configure overall metadata and thresholds</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select 
                                        ...
                                        onValueChange={(val: any) => setData('scale_type', val)}
                                    >
                            ...
                            <div className="flex items-center space-x-2 pt-2">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => loadPreset('necta')}>Load NECTA Preset</Button>
                                <Button type="button" variant="outline" size="sm" onClick={() => loadPreset('competency')}>Load Competency Preset</Button>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <div className="flex justify-end space-x-2">
                        ...
                        <Button type="submit" disabled={processing}>Save Grading Scale</Button>
                    </div>
                </form>
            </div>
        </AppLayout>
